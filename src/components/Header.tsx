...
export default async function Header() {
  ...
  return (
    <div>
      <header className=" h-[50px] md:h-[100px] lg:h-[125px] max-w-[1440px] mx-auto  dark:bg-[#090D1F] flex justify-between items-center px-6  dark:text-white bg-white text-black">
        {/* Left Section */}
        ...
        <nav className="hidden sm:flex items-center space-x-8 sm:pr-[2.25rem]">
          <ul className="flex items-center space-x-8">
            {navLinks.map((item) => (
              <li
                key={item.id}
                className="relative cursor-pointer  hover:underline decoration-2 dark:decoration-white decoration-black underline-offset-[6px] transition-all duration-300 "
              >
